...
% #1 coordinate
% #2 name
% #3 radius (edges are at +- radius in x and y direction)
% a rhomb with all sides having the same length and standing on an edge
\newcommand{\RHOMB}[3]
{
    \path (#1) ++ (0,#3) coordinate(#2_N);
    \path (#1) ++ (0,-#3) coordinate(#2_S);
    \path (#1) ++ (-#3,0) coordinate(#2_W);
    \path (#1) ++ (#3,0) coordinate(#2_E);

    \draw (#2_N) -- (#2_W) -- (#2_S) -- (#2_E) -- (#2_N);
}
